...
if [[ ! -f /opt/postgresql/initialized ]]; then
    ...
    POST_GIS=cat /usr/share/postgresql/9.3/contrib/postgis-2.1/postgis.sql
    REF_SYS=cat /usr/share/postgresql/9.3/contrib/postgis-2.1/spatial_ref_sys.sql
    ...
    su postgres sh -c "/usr/lib/postgresql/9.3/bin/postgres --single  -D  /var/lib/postgresql/9.3/main  -c config_file=/etc/postgresql/9.3/main/postgresql.conf" <<< $POST_GIS
    su postgres sh -c "/usr/lib/postgresql/9.3/bin/postgres --single  -D  /var/lib/postgresql/9.3/main  -c config_file=/etc/postgresql/9.3/main/postgresql.conf" <<< $REF_SYS
