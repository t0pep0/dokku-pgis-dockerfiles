...
if [[ ! -f /opt/postgresql/initialized ]]; then
    ...
    su postgres sh -c "/usr/lib/postgresql/9.3/bin/postgres --single -D  /var/lib/postgresql/9.3/main  -c config_file=/etc/postgresql/9.3/main/postgresql.conf" <<< "\c db; "$POSTGIS_SQL
    SPIRITAL_REF_SYS_SQL=`cat /usr/share/postgresql/9.3/contrib/postgis-2.1/spatial_ref_sys.sql`
    su postgres sh -c "/usr/lib/postgresql/9.3/bin/postgres --single db  -D  /var/lib/postgresql/9.3/main  -c config_file=/etc/postgresql/9.3/main/postgresql.conf" <<< "\c db; " $SPIRITAL_REF_SYS_SQL
